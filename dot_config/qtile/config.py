...
wl_input_rules = {
    "type:pointer": InputConfig(accel_profile='adaptive', pointer_accel=-0.8)
}

@lazy.function
def cycle_windows(qtile) -> None:
    if qtile.current_window is not None and qtile.current_group is not None:
...
keys = [
    ...
    Key([],
        ...
        desc=
        'Save the screens of the currently focused output to the screenshots folder'
        ),
...
def init_widgets_list():
    widgets_list = [
        ...
        widget.WindowName(foreground=colors[6],
                          background=colors[0],
                          padding=0),
        widget.StatusNotifier(background=colors[0], padding=5),
        widget.Sep(linewidth=0,
                   padding=6,
                   foreground=colors[0],
                   background=colors[0]),
        ...
        widget.GenPollText(
            ...
            func=lambda: subprocess.check_output(
                os.path.expanduser("~/.local/bin/statusbar/idleinhibit")
            ).decode(),
            mouse_callbacks={
                'Button1':
                lambda: qtile.cmd_spawn(os.path.expanduser(
                    "~/.local/bin/statusbar/idleinhibit toggle"),
                                        shell=True)
            }),
        widget.TextBox(text='',
                       font="Ubuntu Mono",
...
def init_widgets_screen2():
    ...
    del widgets_screen2[
        9:10]  # Slicing removes unwanted widgets (systray) on Monitor 2
...
def init_screens():
    return [
        Screen(
            ...
            wallpaper='/usr/share/wallpapers/garuda-wallpapers/since-sgs.png'),
        Screen(
...
floating_layout = layout.Floating(float_rules=[
    ...
    *layout.Floating.default_float_rules,
    ...
    Match(title='Discord'),  # Discord
    Match(title='Element Nightly'),  # Element
    Match(title='Wizard101'),  # Wiz
    Match(title='Lutris'),
    Match(wm_class='steam_proton'),  # Proton
